CREATE TEMP TABLE "legacy_client_engagements" AS
...
) THEN CAST(concat('Legacy clients are not migrated to Engagements yet. Run db:migrate:engagements before deploying this migration.') AS integer) END;
--> statement-breakpoint
